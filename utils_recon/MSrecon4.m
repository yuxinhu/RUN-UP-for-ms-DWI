function [imc,im] = MSrecon4(k,sens,iter,method,partialF,aveg,allnex,lambda,k0,l1_flag)
...
%   method: MUSE, POCSMUSE, POCS-ICE, POCS-MUSSELS, POCS-MUSSELS-TV, LLR, LLR-VCS, LLR_TWOC. Notice that LLR_TWOC (low rank + sparse) requires our modified BART.
...
if allnex
    ...
    if strcmp('MUSE', method)
        fprintf('Does not support allnex for MUSE reconstruction \n');
    end
    k = k(:,:,:,:);
    k0 = k0(:,:,:,:);
end
...
switch method
    case 'MUSE'
        im_pocsmuse = zeros(nx,ny,nex);
        for n = 1 : nex
            [im_muse(:,:,n),~] = MUSE(k(:,:,:,:,n),sens);
        end
        imc = im_muse;
    ...
    case 'POCSMUSE'
        ...
        for n = 1 : nex
            if nargin < 9
                ...
                [im_pocsmuse(:,:,n),~] = POCSMUSE(k(:,:,:,:,n),permute(sens,[1 2 4 3]),iter);
            else
                [im_pocsmuse(:,:,n),~] = POCSMUSE(k(:,:,:,:,n),permute(sens,[1 2 4 3]),iter,k0(:,:,:,n));
            end
        end
        imc = im_pocsmuse;
    ...
    case 'POCS-MUSSELS'
        im_mussels = zeros(nx, ny, nshot, nex);
        for n = 1 : nex
            im_mussels(:,:,:,n) = MUSSELS(k(:,:,:,:,n), sens, iter, lambda);
        end
        imc = im_mussels(:,:,:,:);

    case 'POCS-MUSSELS-TV'
        im_mussels = zeros(nx, ny, nshot, nex);
        for n = 1 : nex
            im_mussels(:,:,:,n) = MUSSELS_TV(k(:,:,:,:,n), sens, iter, lambda);
        end
        imc = im_mussels(:,:,:,:);
    case 'LLR'
