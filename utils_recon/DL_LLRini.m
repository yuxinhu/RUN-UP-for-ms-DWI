% DWI reconstruction: for (1) initialization of SPA-LLR (use SENSE) and (2) prep of deep learning reconstruction (use shot-LLR).
...
for r = 1 : length(p.r)
    ...
    if p.nx > 300 % If breast data, then use low-resolution b=0 as sensitivity map
        acs = min(p.extraL*2,48);
        sens = klb_cc(:,:,:,1).*0; % use the first nex
        sens(:,p.ny/2-acs/2+1:p.ny/2+acs/2,:) = klb_cc(:,p.ny/2-acs/2+1:p.ny/2+acs/2,:,1);
        sens = ifft2c(sens);
        sens = sens ./ repmat(sos(sens),[1 1 p.v]);
    else
        [sens ~] = squeeze(bart('ecalib -r 24 -m 1', permute(klb_cc(:,:,:,1),[1 2 4 3]))); 
    end  
    ...
    if ismember(p.dir, p.b0)        
        ...
        if strcmp(p.reconmethod,'LLR')
            ...
            [im_ini,LLR(:,:,s,r)] = MSrecon4(khb_cc,sens,p.iter,'LLR',0.5+p.extraL/p.ny,p.caverage,p.allnex,p.lambda);
